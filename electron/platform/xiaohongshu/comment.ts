...
const logger = new Logger('XiaohongshuComment');

const RATE_LIMIT = { hourly: 5, daily: 20, burst: 2 } as const;
...
const COMMENT_SELECTORS = {
  ...
  commentItem: '[class*="comment-item"], [class*="CommentItem"], .comment-inner',
  replyBtn: 'button:has-text("回复"), [class*="reply-btn"], .reply-btn',
  replyInput: [
    'textarea[placeholder*="回复"]',
    'textarea[placeholder*="写下你的回复"]',
    '[class*="reply-input"] textarea',
  ],
  deleteBtn: 'button:has-text("删除"), [class*="delete-btn"], .delete-comment',
...
async function navigateToNote(page: Page, noteId: string): Promise<boolean> {
  try {
    ...
    logger.error('导航到笔记页面失败', { noteId, error });
    return false;
  }
}
...
async function expandCommentSection(page: Page): Promise<boolean> {
  const expandBtn = page.locator('button:has-text("评论"), [class*="comment"] [class*="expand"], [class*="toggle-comment"]').first();
...
export async function postComment(
  ...
  try {
    ...
    if (!(await navigateToNote(page, noteId))) {
      return { success: false, message: `无法导航到笔记页面: ${noteId}` };
    }

    await expandCommentSection(page);
    ...
    logger.info('评论发布成功', { noteId, comment });
    return { success: true, message: '评论发布成功' };
  } catch (error) {
    logger.error('评论发布失败', { noteId, error });
...
export async function fetchComments(
  ...
  try {
    if (!(await navigateToNote(page, noteId))) return [];
    await expandCommentSection(page);
    ...
    for (let i = 0; i < Math.min(commentItems.length, maxCount); i++) {
      ...
      if (text) {
        comments.push({ id: `xhs_cmt_${i}_${Date.now()}`, content: text.trim(), author: '' });
      }
    }

    logger.info(`获取到 ${comments.length} 条评论`, { noteId });
    return comments;
  } catch (error) {
    logger.error('获取评论列表失败', { noteId, error });
    return [];
  }
}
...
export async function replyComment(
  ...
  try {
    ...
    await page.waitForTimeout(3000);
    recordCall(rateTracker);
    logger.info('回复评论成功', { noteId, content });
    return { success: true, message: '回复评论成功' };
  } catch (error) {
    logger.error('回复评论失败', { noteId, error });
...
export async function deleteComment(
  ...
  try {
    ...
    logger.info('评论已删除', { noteId });
    return { success: true, message: '评论已删除' };
  } catch (error) {
    logger.error('删除评论失败', { noteId, error });
...
export async function postCommentWithRetry(
  ...
  for (let i = 0; i < maxRetries; i++) {
    if (i > 0) {
      const backoff = Math.min(4000 * Math.pow(2, i - 1), 40000);
...
export async function postCommentFull(
  ...
  try {
    ...
    return await postCommentWithRetry(page, { accountId, videoId: noteId, comment });
  } catch (error) {
