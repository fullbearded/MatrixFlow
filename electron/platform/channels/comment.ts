...
const RATE_LIMIT = { hourly: 8, daily: 30, burst: 2 } as const;
...
const COMMENT_SELECTORS = {
  ...
  commentItem: '[class*="comment-item"], [class*="CommentItem"], .comment-card',
...
async function navigateToVideo(page: Page, videoId: string): Promise<boolean> {
  try {
    const videoUrl = `https://channels.weixin.qq.com/video/${videoId}`;
...
export async function fetchComments(
  ...
  try {
    ...
    for (let i = 0; i < Math.min(commentItems.length, maxCount); i++) {
      ...
      if (text) {
        comments.push({ id: `channels_cmt_${i}_${Date.now()}`, content: text.trim(), author: '' });
      }
    }
