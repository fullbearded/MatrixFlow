...
const logger = new Logger('KuaishouComment');

const RATE_LIMIT = { hourly: 6, daily: 25, burst: 2 } as const;
...
const COMMENT_SELECTORS = {
  ...
  commentItem: '[class*="comment-item"], [class*="CommentItem"], .comment__item',
...
async function navigateToVideo(page: Page, videoId: string): Promise<boolean> {
  try {
    const videoUrl = `https://www.kuaishou.com/short-video/${videoId}`;
...
export async function fetchComments(
  ...
  try {
    ...
    for (let i = 0; i < Math.min(commentItems.length, maxCount); i++) {
      ...
      if (text) {
        comments.push({ id: `ks_cmt_${i}_${Date.now()}`, content: text.trim(), author: '' });
      }
    }
